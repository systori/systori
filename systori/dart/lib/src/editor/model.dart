...
class ModelState {
  ...
  static Map<String, dynamic> initialCommitted(Model model) => model.hasPK
...
abstract class Model extends HtmlElement {
  ...
  HtmlElement getView(String field) => editor.querySelector(".${field}");
...
class Repository {
  ...
  Future<Map> save(int jobId, Map data) async {
    var response = await HttpRequest.request("/api/editor/save/job/$jobId/",
        method: "POST",
        requestHeaders: headers,
        sendData:
            JSON.encode(data, toEncodable: (o) => o is Set ? o.toList() : o));
  ...
  Future<List<Map>> search(Map<String, String> criteria) async {
    var response = await HttpRequest.request("/api/editor/search",
        method: "POST",
        requestHeaders: headers,
  ...
  Future<String> clone(Map<String, String> params) async {
    var response = await HttpRequest.request("/api/editor/clone",
        method: "POST", requestHeaders: headers, sendData: JSON.encode(params));
...
class ChangeManager {
  ...
  Future save() {
    if (saving == null) {
      ...
      if (saveRequest == null) saveRequest = new Completer();
