...
CSRFINPUT = re.compile(
    rb"(<input type='hidden' name='csrfmiddlewaretoken' value=')[0-9A-Za-z]+(' />)"
)
...
def write_test_html(path, name, html):
    ...
    with open(file_path, "wb") as test_file:
        ...
        test_file.write(
            b'<link rel="x-dart-test" href="' + name.encode() + b'_test.dart">\n'
        )
        test_file.write(b'<script src="packages/test/dart.js"></script>\n')
        test_file.write(b"<!--\n")
        test_file.write(
            b'<script type="application/dart" src="'
            + name.encode()
            + b'_test.dart"></script>\n'
        )
        test_file.write(b"-->\n")
...
def create_data():
    ...
    for groupset in range(3):
        ...
        group = GroupFactory(
            name="Main Group {}1".format(groupset), parent=data.big_job
        )
        groups = []
...
def generate_amount_test_html(data):
    form = PaymentRowForm(
        initial={"jobs": [data.job1], "job": data.job1, "split": A(D("4800"), D("912"))}
    )
...
def generate_pages():
    ...
    job_editor = client.get(
        reverse("job.editor", args=[data.project.id, data.job1.id]), HTTP_HOST=host
    )
    ...
    payment_create = client.get(
        reverse("payment.create", args=[data.project.id]), HTTP_HOST=host
    )
    write_test_html("apps", "payment_editor", payment_create.content)

    adjustment_create = client.get(
        reverse("adjustment.create", args=[data.project.id]), HTTP_HOST=host
    )
    write_test_html("apps", "adjustment_editor", adjustment_create.content)

    refund_create = client.get(
        reverse("refund.create", args=[data.project.id]), HTTP_HOST=host
    )
