...
\tabucline \\
Invoice Total & {{project.billable_gross_total|money|tex_escape|trim}} € & {{project.billable_total|money|tex_escape|trim}} € & {{project.billable_tax_total|money|tex_escape|trim}} €\\[5pt]
{% for credit in project.account.credits %}
{% if credit.is_discount %}
Discount applied & {{ credit.amount|money|tex_escape|trim }} € & {{ credit.amount_base|money|tex_escape|trim }} € & {{ credit.amount_tax|money|tex_escape|trim }} €\\[5pt]
{% else %}
Your Payment on {{ credit.transaction.date_recorded|date:"DATE_FORMAT" }} & {{ credit.amount|money|tex_escape|trim }} € & {{ credit.amount_base|money|tex_escape|trim }} € & {{ credit.amount_tax|money|tex_escape|trim }} €\\[5pt]
{% endif %}
{% endfor %}
\bfseries{Remaining amount} & \bfseries{ {{project.account.balance|money|tex_escape|trim}} €} & \bfseries{ {{project.account.balance_base|money|tex_escape|trim}} €} & \bfseries{ {{project.account.balance_tax|money|tex_escape|trim}} €}\\[5pt]
