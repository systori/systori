...
class TestProjectViews(ClientTestCase):
    ...
    def test_update_project_initial(self):
        ...
        self.assertEqual(200, response.status_code)
        self.assertEqual(
            "01.01.001", str(response.context["form"].initial["structure"])
        )

    def test_update_project_success(self):
        data = {"name": "updated project", "structure": "0.0.0"}
        project = ProjectFactory(with_job=True)
    ...
    def test_update_project_invalid(self):
        data = {"name": "updated project", "structure": "0.0.0.0"}
        project = ProjectFactory(with_job=True)
        response = self.client.post(reverse("project.edit", args=[project.pk]), data)
        self.assertEqual(200, response.status_code)
        form = response.context["form"]
    ...
    def test_view_project(self):
        ...
        proposal = ProposalFactory(project=project, letterhead=LetterheadFactory())
    ...
    def test_phase_transition(self):
        ...
        for action, phase in [
            ("begin_tendering", Project.TENDERING),
            ("begin_planning", Project.PLANNING),
        ]:
...
class TestJobSiteViews(ClientTestCase):
    ...
    def test_create_jobsite_invalid(self):
        data = {"name": "", "address": "", "city": "", "postal_code": ""}
        response = self.client.post(
            reverse("jobsite.create", args=[self.project.pk]), data
        )
    ...
    def test_update_jobsite_initial(self):
        jobsite = JobSiteFactory(project=self.project)
        response = self.client.get(
            reverse("jobsite.edit", args=[self.project.pk, jobsite.pk])
        )
        self.assertEqual(200, response.status_code)
    ...
    def test_update_jobsite_success(self):
        ...
        response = self.client.post(
            reverse("jobsite.edit", args=[self.project.pk, jobsite.pk]), data
        )
        self.assertEqual(302, response.status_code)
    ...
    def test_delete_jobsite(self):
        ...
        self.assertEqual(1, JobSite.objects.count())
        response = self.client.get(
            reverse("jobsite.delete", args=[self.project.pk, jobsite.pk])
        )
        self.assertEqual(200, response.status_code)
        response = self.client.post(
            reverse("jobsite.delete", args=[self.project.pk, jobsite.pk])
    ...
    def test_activity_days_dailyplans_jobsite(self):
        ...
        self.assertEqual(
            date(2015, 1, 1), response.context["jobsites"][0].first_day
        )  # check annotate
        self.assertEqual(
            date(2018, 2, 1), response.context["jobsites"][1].last_day
        )  # cehck annotate
    ...
    def test_project_dailyplans_view(self):
        ...
        response = self.client.get(
            reverse("project.dailyplans", args=[self.project.pk]), {}
        )
        self.assertEqual(
            response.context["dailyplans"][0].worker_count, 2
        )  # 2 workers on first dailyplan
        self.assertEqual(
            response.context["workers_summary"][0][1], 4
        )  # worker1 on 4 dailyplans
        self.assertEqual(
            response.context["total_dailyplans"], 4
        )  # 4 dailyplans existing
        self.assertEqual(
