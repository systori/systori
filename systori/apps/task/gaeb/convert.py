...
class Export:
    ...
    def project(self, project, jobs=None):
        ...
        return self.e.GAEB(
            self.e.PrjInfo(self.e.LblPrj(project.name)),
            self.e.Award(
                self.e.BoQ(
                    self.e.BoQInfo(
                        *(
                    ...
                    self.e.BoQBody(*(self.group(job) for job in jobs)),
                )
            ),
        )
    ...
    def group(self, group):
        ...
        return self.e.BoQCtgy(self.e.LblTx(group.name), self.e.BoQBody(*body))
    ...
    def task(self, task):
        elements = [
            ...
            self.e.Description(
                self.e.CompleteText(
                    self.e.OutlineText(self.e.OutlTxt(self.e.TextOutlTxt(task.name))),
                    self.e.DetailTxt(self.e.Text(task.description)),
                )
            ),
        ]
...
class Import:
    ...
    def parse(self, file):
        try:
            ...
            if self.project.structure.maximum_depth != self.max_group_depth:
                self.error(
                    _("Project and imported jobs do not have compatible structure.")
                )
        except:
            self.error(
                _("File '%s' can't be imported. Please contact support.") % file.name
            )
        return self
    ...
    def group(self, parent, group_element):
        try:
            if isinstance(parent, Project):
                group = Job(
                    name=" ".join(group_element.LblTx.xpath(".//text()")),
                    project=parent,
                )
                group.job = group
                ...
                group = Group(
                    name=" ".join(group_element.LblTx.xpath(".//text()")), parent=parent
                )
            self.max_group_depth = max(self.max_group_depth, group.depth)
            self.objects.append(group)
            for sub_group_element in group_element.BoQBody.iterchildren(
                self.ns + "BoQCtgy"
            ):
                self.group(group, sub_group_element)
            if hasattr(group_element.BoQBody, "Itemlist"):
                for task_element in group_element.BoQBody.Itemlist.iterchildren(
                    self.ns + "Item"
                ):
    ...
    def task(self, group, task_element):
        try:
            ...
            for (
                text_node
            ) in task_element.Description.CompleteText.DetailTxt.getchildren():
                    ...
                    for child in p.getchildren():
                        if child.tag == self.ns + "span":
                            ...
                            task.description += str(
                                _(" Info: Picture was in Imported File.")
                            )
            for (
                text_node
            ) in task_element.Description.CompleteText.OutlineText.getchildren():
