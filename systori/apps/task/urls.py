...
urlpatterns = patterns('',
                       ...
                       url(r'^job-(?P<pk>\d+)/transition/(?P<transition>\w+)$', office_auth(JobTransition.as_view()),
                           name='job.transition'),
                       url(r'^job-(?P<pk>\d+)/estimate/(?P<action>\w+)$',
                           office_auth(JobEstimateModification.as_view()), name='job.estimate'),
