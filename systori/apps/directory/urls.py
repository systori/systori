...
urlpatterns = patterns('',
                       ...
                       url(r'^contact-(?P<pk>\d+)/delete$', office_auth(ContactDelete.as_view()),
                           name='contact.delete'),

                       url(r'^project-(?P<project_pk>\d+)/contact/add$', office_auth(ProjectContactAdd.as_view()),
                           name='project.contact.add'),
                       url(r'^project-(?P<project_pk>\d+)/contact/create$', office_auth(ProjectContactCreate.as_view()),
                           name='project.contact.create'),
                       url(r'^project-(?P<project_pk>\d+)/contact-(?P<pk>\d+)/remove$',
                           office_auth(ProjectContactRemove.as_view()), name='project.contact.remove'),
                       url(r'^project-(?P<project_pk>\d+)/contact-(?P<pk>\d+)/billable$',
                           office_auth(ProjectContactSetBillable.as_view()), name='project.contact.billable'),
                       )
