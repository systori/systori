    ...
    for job, refund, refund_credit in jobs:
        ...
        if refund_credit.gross > 0:
            ...
            transaction.credit(
                job.account,
                refund_credit.net,
                entry_type=Entry.REFUND_CREDIT,
                job=job,
                value_type=Entry.NET,
            )
...
def create_chart_of_accounts(self=None):
    ...
    self.promised_payments = Account.objects.create(
        name="Promised Payments",
        account_type=Account.LIABILITY,
        code=SKR03_PROMISED_PAYMENTS_CODE,
    )
    self.partial_payments = Account.objects.create(
        name="Partial Payments",
        account_type=Account.LIABILITY,
        code=SKR03_PARTIAL_PAYMENTS_CODE,
    )
    self.tax_payments = Account.objects.create(
        name="Tax Payments",
        account_type=Account.LIABILITY,
        code=SKR03_TAX_PAYMENTS_CODE,
    )

    self.income = Account.objects.create(
        name="Income", account_type=Account.INCOME, code=SKR03_INCOME_CODE
    )
    self.cash_discount = Account.objects.create(
        name="Cash Discount", account_type=Account.INCOME, code=SKR03_CASH_DISCOUNT_CODE
    )

    self.bank = Account.objects.create(
        name="Default Bank Account",
        account_type=Account.ASSET,
        asset_type=Account.BANK,
        code=SKR03_BANK_CODE,
    )
