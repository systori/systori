...
class TestTransactionsTable(TestCase):
    ...
    def tbl(self, invoice_txn=None, jobs=None):
        if not invoice_txn:
            invoice_txn = debit_jobs(
                [(self.job, A(0), Entry.WORK_DEBIT)], transacted_on=days_ago(0)
            )
        if not jobs:
            jobs = [self.job]
    ...
    def test_one_invoice_paid_with_adjustment(self):
        debit_jobs([(self.job, A(1190), Entry.WORK_DEBIT)], transacted_on=days_ago(2))
        credit_jobs(
            [(self.job, A(595), A(), A(595))], D(595), transacted_on=days_ago(1)
        )
        self.assertEqual(
            self.tbl(),
            [
    ...
    def test_one_invoice_paid_with_discount(self):
        debit_jobs([(self.job, A(1190), Entry.WORK_DEBIT)], transacted_on=days_ago(2))
        credit_jobs(
            [(self.job, A(595), A(595), A())], D(595), transacted_on=days_ago(1)
        )
        self.assertEqual(
            self.tbl(),
            [
    ...
    def test_invoices_with_adjustment_and_payment_project_41_issue_2016_02_24(self):
        ...
        credit_jobs(
            [(self.job, A(1190), A(0), A(0))], D(1190), transacted_on=days_ago(5)
        )
        ...
        txn = debit_jobs(
            [(self.job, A(119), Entry.WORK_DEBIT)], transacted_on=days_ago(1)
        )
    ...
    def test_project_150_issue_2016_01_17(self):
        ...
        debit_jobs(
            [(self.job, A(10569.95), Entry.WORK_DEBIT)], transacted_on=days_ago(3)
        )
        debit_jobs(
            [(self.job, A(14045.95), Entry.WORK_DEBIT)], transacted_on=days_ago(2)
        )

        credit_jobs(
            [(self.job, A(28560), A(0), A(0))], D(28560), transacted_on=days_ago(3)
        )
        credit_jobs(
            [(self.job, A(8925), A(0), A(1644.95))], D(8925), transacted_on=days_ago(2)
        )
    ...
    def test_project_143_having_open_claim_2016_04_06(self):
        ...
        self.assertEqual(
            self.tbl(dtxn, [self.job, self.job2, self.job3, self.job4, self.job5]),
            [
                ("", "net", "tax", "gross"),
