...
def create_account_for_job(job):
    ...
    int(
        job.id
    )  # raises exception if id is not an int (eg. job hasn't been saved()'ed yet)
    ...
    if code > DEBTOR_CODE_RANGE[1]:
        raise ValueError(
            ...
            % (code, DEBTOR_CODE_RANGE[1])
        )
    return Account.objects.create(
        account_type=Account.ASSET, asset_type=Account.RECEIVABLE, code=str(code)
    )
