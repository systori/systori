...
def split_gross_entries(apps, schema_editor):
    ...
    from systori.apps.accounting.constants import (
        TAX_RATE,
    ...
    for company in Company.objects.all():
        ...
        values_types = (
            ...
            (Entry.TAX, (SKR03_TAX_PAYMENTS_CODE,)),
            (Entry.GROSS, (SKR03_PROMISED_PAYMENTS_CODE,)),
        )
            ...
            try:
                if "total_base" not in proposal.json and "jobs" not in proposal.json:
                    ...
                    proposal.json["estimate_total"] = Amount(
                        ...
                        proposal.json["total_gross"] - proposal.json["total_net"],
                    )
                    proposal.json["jobs"] = []

                else:
                    proposal.json["estimate_total"] = Amount(
                        proposal.json["total_base"], proposal.json["total_tax"]
                    )
                ...
                for job in proposal.json["jobs"]:
                    ...
                    if "id" in job:
                        ...
                        job_obj = Job.objects.filter(
                            project=proposal.project, name=job["name"]
                        ).get()
...
def migrate_invoices(apps, schema_editor):
    ...
    for company in Company.objects.all():
        ...
        for invoice in Invoice.objects.all():
            ...
            invoice.json["debit"] = Amount(
                invoice.json["debit_net"], invoice.json["debit_tax"]
            )
    ...
    operations = [
        migrations.AddField(
            ...
            field=models.CharField(
                max_length=32,
                default="gross",
                choices=[("net", "Net"), ("tax", "Tax"), ("gross", "Gross")],
                verbose_name="Value Type",
            ),
            preserve_default=False,
        ),
        migrations.RenameField(model_name="entry", old_name="amount", new_name="value"),
        migrations.AlterField(
            model_name="entry",
            name="value",
            field=models.DecimalField(
                verbose_name="Value", decimal_places=2, max_digits=14
            ),
        ),
        migrations.AlterField(
            model_name="entry",
        ...
        migrations.RemoveField(model_name="entry", name="tax_rate"),
        migrations.AlterField(
