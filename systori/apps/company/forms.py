...
class CompanyForm(ModelForm):
    ...
    def save(self, commit=True):
        ...
        if adding:
            company.activate()
            Worker.objects.create(company=company, user=self.user, is_owner=True)
            ...
            letterhead = clean_letterhead_pdf(
                File(open(demo_letterhead, "rb")), save=True
            )
