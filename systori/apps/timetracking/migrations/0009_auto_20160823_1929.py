    ...
    for company in Company.objects.all():
        ...
        for timer in Timer.objects.all():
            ...
            if timer.start.hour < 7:
                timer.start = timer.start.replace(hour=7, minute=0, second=0)
                if timer.end:
                    timer.duration = None
                timer.save()
            ...
            if timer.duration >= 60 * 60 * 9:
                ...
                timer.end = timer.end.replace(hour=9, minute=0, second=0)
                timer.duration = None
                timer.save()
                ...
                timer.id = None
                timer.start = timer.start.replace(hour=9, minute=30, second=0)
                timer.end = timer.end.replace(hour=12, minute=30, second=0)
                timer.duration = None
                timer.save()
                ...
                timer.id = None
                timer.start = timer.start.replace(hour=13, minute=0, second=0)
                timer.end = old_end
                timer.duration = None
                timer.save()
...
class Migration(migrations.Migration):
    ...
    dependencies = [("timetracking", "0008_auto_20160802_0820")]
