...
class UserReportViewTest(ClientTestCase):
    ...
    def test_default_case(self):
        response = self.client.get(
            reverse("timetracking_worker", args=[self.worker.pk])
        )
    ...
    def test_user_from_another_company_cannot_be_viewed(self):
        ...
        another_worker = UserFactory(
            company=another_company, password=self.password
        ).access.first()
        response = self.client.get(
            reverse("timetracking_worker", args=[another_worker.pk])
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_custom_report_period_en(self):
        response = self.client.get(
            reverse("timetracking_worker", args=[self.worker.pk]), {"period": "01/2010"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class TimerDeleteViewTest(ClientTestCase):
    def test_delete_single_timer(self):
        ...
        post_response = self.client.post(
            reverse("timer.delete", args=[timer0.id]), follow=True
        )
        self.assertRedirects(
            post_response,
            reverse("timetracking_worker", args=[self.worker.id]),
            status_code=302,
        )
...
class TimerVacationScheduleTest(ClientTestCase):
    def setUp(self):
        super().setUp()
        self.timer0 = Timer.objects.create(
            worker=self.worker,
            started=NOW + timedelta(days=1),
            stopped=NOW + timedelta(days=1, hours=8),
            kind="vacation",
        )
        self.timer1 = Timer.objects.create(
            worker=self.worker,
            started=NOW + timedelta(days=2),
            stopped=NOW + timedelta(days=2, hours=8),
            kind="vacation",
        )

    def test_get_vacation_schedule(self):
        response = self.client.get(
            reverse("timetracking.vacation.schedule")
        )  # renders?
        schedule = Timer.objects.get_vacation_schedule(year=NOW.year)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            schedule[self.worker]["available"], 13440
        )  # 2 days vacation, 28 days expected to be available
    ...
    def test_get_filtered_vacation(self):
        self.assertEqual(
            self.client.get(
                reverse("timetracking.vacation.schedule", args=[2017])
            ).status_code,
            200,
        )
