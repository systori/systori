...
class UserStatusesTest(TestCase):
    ...
    def test_get_worker_statuses(self):
        ...
        worker1_timer = Timer.objects.create(
            worker=self.worker1, started=morning, kind=Timer.WORK
        )
        Timer.objects.create(
            worker=self.worker1,
            started=yesterday,
            stopped=yesterday + timedelta(hours=8),
            kind=Timer.WORK,
        )
        Timer.objects.create(
            worker=self.worker2,
            started=morning,
            stopped=morning + timedelta(hours=2),
            kind=Timer.WORK,
        )
...
class TimeSpanTest(TestCase):
    ...
    def assertSpan(self, expected, start, end):
        self.assertEquals(
            [(time(*span[0]), time(*span[1])) for span in expected],
            list(utils.get_timespans_split_by_breaks(start, end, self.breaks)),
        )

    def test_simple(self):
        self.assertSpan(
            [((7, 00), (9, 00)), ((9, 30), (12, 30)), ((13, 00), (16, 00))],
            time(7),
            time(16),
        )
    ...
    def test_simple_overtime(self):
        self.assertSpan(
            [((7, 00), (9, 00)), ((9, 30), (12, 30)), ((13, 00), (18, 00))],
            time(7),
            time(18),
        )

    def test_start_at_break(self):
        self.assertSpan([((9, 30), (12, 30)), ((13, 00), (16, 00))], time(9), time(16))

    def test_late_start(self):
        self.assertSpan([((14, 00), (16, 00))], time(14), time(16))

    def test_early_finish(self):
        self.assertSpan([((8, 00), (9, 00)), ((9, 30), (10, 00))], time(8), time(10))
    ...
    def test_early_start_early_break(self):
        self.assertSpan(
            [((5, 00), (9, 00)), ((9, 30), (11, 30))], time(5), time(11, 30)
        )
