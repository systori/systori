...
class TasksTest(TestCase):
    ...
    def test_start_or_stop_work_breaks(self):
        ...
        def run():
            ...
            previous1 = worker1.timers.filter(stopped__isnull=False).order_by('stopped').first()
            company2.activate()
            timer2 = worker2.timers.latest('started')
            previous2 = worker2.timers.filter(stopped__isnull=False).order_by('stopped').first()
    ...
    def test_stop_abandoned_timers(self):
        ...
        worker1 = UserFactory(company=company1).access.first()
        Timer.objects.create(worker=worker1, started=timezone.now() - timedelta(hours=7))

        company2 = CompanyFactory(schema='testcompany2')
        worker2 = UserFactory(company=company2).access.first()
        Timer.objects.create(worker=worker2, started=timezone.now() - timedelta(hours=5))
