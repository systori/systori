...
class DocumentTestCase(ClientTestCase):
    ...
    def setUp(self):
        ...
        self.task = TaskFactory(
            name="Task #1 Fixed Price", qty=10, complete=5, price=96, group=self.group
        )
...
class ProposalViewTests(DocumentTestCase):
    ...
    def test_serialize_n_render_proposal(self):
        ...
        response = self.client.post(
            reverse("proposal.create", args=[self.project.id]), data
        )
        ...
        response = self.client.get(
            reverse(
                "proposal.pdf",
                args=[self.project.id, "print", Proposal.objects.first().id],
            ),
            {"with_lineitems": True},
        )
        self.assertEqual(200, response.status_code)
        ...
        response = self.client.get(
            reverse(
                "proposal.pdf",
                args=[self.project.id, "email", Proposal.objects.first().id],
            ),
            {"with_lineitems": True},
        )
        self.assertEqual(200, response.status_code)
    ...
    def test_serialize_n_render_small_structure(self):
        ...
        extractedText = (
            PdfFileReader(BytesIO(response.content)).getPage(0).extractText()
        )
    ...
    def test_serialize_n_render_big_structure(self):
        ...
        self.task = TaskFactory(
            ...
            complete=5,
            price=96,
            group=self.job.all_groups.order_by("id").last(),
        )
    ...
    def test_update_proposal(self):
        ...
        response = self.client.post(
            reverse("proposal.update", args=[self.project.id, proposal.id]), data
        )
        self.assertEqual(302, response.status_code)
    ...
    def test_serialize_n_render_with_lineitems(self):
        ...
        for text in ["Proposal", "hello", "bye", self.job.name, self.task.name]:
            self.assertTrue(text in extractedText)
        extractedText = (
            PdfFileReader(BytesIO(response.content)).getPage(1).extractText()
        )
    ...
    def get_rendered_doc(self):
        ...
        response = self.client.get(
            reverse(
                "proposal.html", args=[self.project.id, Proposal.objects.first().id]
            )
        )
...
class InvoiceViewTests(DocumentTestCase):
    ...
    def test_happy_path_create_update_and_render(self):
        ...
        data.update(self.make_management_form())
        response = self.client.post(
            reverse("invoice.create", args=[self.project.id]), data
        )
        self.assertEqual(302, response.status_code)
        ...
        response = self.client.get(
            reverse(
                "invoice.pdf",
                args=[self.project.id, "print", Invoice.objects.first().id],
            )
        )
        self.assertEqual(200, response.status_code)
        ...
        response = self.client.post(
            reverse("invoice.update", args=[self.project.id, invoice.id]), data
        )
        self.assertEqual(302, response.status_code)
        self.assertRedirects(response, reverse("project.view", args=[self.project.id]))
    ...
    def get_rendered_doc(self, jobs):
        ...
        response = self.client.get(
            reverse("invoice.html", args=[self.project.id, Invoice.objects.first().id])
        )
        return clean_doc(response.content)
    ...
    def test_render_time_and_materials_invoice(self):
        ...
        LineItemFactory(
            name="TM Lineitem", qty=20, price=10, total=200, expended=20, task=tm_task
        )
        text = self.get_rendered_doc(
            {
                "job-0-is_invoiced": "True",
...
class PaymentViewTests(DocumentTestCase):
    ...
    def test_happy_path_create_update_and_render(self):
        ...
        data.update(self.make_management_form())
        response = self.client.post(
            reverse("payment.create", args=[self.project.id]), data
        )
        self.assertEqual(302, response.status_code)
        ...
        response = self.client.get(
            reverse(
                "payment.pdf",
                args=[self.project.id, "print", Payment.objects.first().id],
            )
        )
        self.assertEqual(200, response.status_code)
        ...
        data.update(self.make_management_form())
        response = self.client.post(
            reverse("payment.update", args=[self.project.id, payment.id]), data
        )
        self.assertEqual(302, response.status_code)
        self.assertRedirects(response, reverse("project.view", args=[self.project.id]))
...
class AdjustmentViewTests(DocumentTestCase):
    ...
    def test_happy_path_create_update_and_render(self):
        ...
        data.update(self.make_management_form())
        response = self.client.post(
            reverse("adjustment.create", args=[self.project.id]), data
        )
        self.assertEqual(302, response.status_code)
        ...
        response = self.client.get(
            reverse(
                "adjustment.pdf",
                args=[self.project.id, "print", Adjustment.objects.first().id],
            )
        )
        self.assertEqual(200, response.status_code)
        ...
        response = self.client.post(
            reverse("adjustment.update", args=[self.project.id, adjustment.id]), data
        )
        self.assertEqual(302, response.status_code)
        self.assertRedirects(response, reverse("project.view", args=[self.project.id]))
...
class RefundViewTests(DocumentTestCase):
    ...
    def test_happy_path_create_update_and_render(self):
        ...
        response = self.client.get(
            reverse(
                "refund.pdf", args=[self.project.id, "print", Refund.objects.first().id]
            )
        )
        self.assertEqual(200, response.status_code)
        ...
        self.assertEqual(
            refund.json["refund_total"], Amount(Decimal("1.0"), Decimal("1.0"))
        )
        self.assertEqual(
            refund.json["credit_total"], Amount(Decimal("1.0"), Decimal("1.0"))
        )
        ...
        self.assertEqual(
            refund.json["refund_total"], Amount(Decimal("2.0"), Decimal("2.0"))
        )
        self.assertEqual(
            refund.json["credit_total"], Amount(Decimal("2.0"), Decimal("2.0"))
        )
...
class LetterheadCreateTests(DocumentTestCase):
    def test_post(self):
        ...
        with open(
            "systori/apps/document/test_data/letterhead.pdf", "rb"
        ) as lettehead_pdf:
            response = self.client.post(
        ...
        self.assertRedirects(
            response,
            reverse("letterhead.update", args=[Letterhead.objects.latest("pk").pk]),
        )
...
class TimesheetViewTests(ClientTestCase):
    ...
    def test_correction(self):
        ...
        sheet = Timesheet.objects.get()
        self.client.post(
            reverse("timesheet.update", args=[sheet.pk]),
            data={
