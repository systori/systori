...
def parse_date(str_date):
    return (
        date_format(date(*map(int, str_date.split("-"))), use_l10n=True)
        if str_date
        else None
    )
...
class BaseRowIterator:
    ...
    def get_task_context(self, task, **kwargs):
        ...
        if task.get("variant_group"):
            if task["variant_serial"] == 0:
                kwargs["name"] = _("Variant {}.0: {}").format(
                    task["variant_group"], task["name"]
                )
            else:
                kwargs["name"] = _(
                    "Variant {}.{}: {} - Alternative for Variant {}.0"
                ).format(
                    task["variant_group"],
                    task["variant_serial"],
                    task["name"],
    ...
    def get_subtotal_context(self, group, **kwargs):
        kwargs.update({"code": group["code"], "name": group["name"]})
        return kwargs
