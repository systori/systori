...
from .style import NumberedSystoriDocument, stylesheets, TableFormatter, ContinuationTable
...
def collate_payments(invoice, available_width):
    ...
    for payment in invoice['transactions']:
        ...
        if payment['type'] == 'payment':
            ...
            row[0] = Paragraph(_('Your Payment on') +' ' + received_on, stylesheets['OpenSans']['Normal'])
...
def render(project, format):
    ...
    with BytesIO() as buffer:
        ...
        flowables = [
            ...
            Paragraph(_("Itemized listing {}").format(date.today), stylesheets['OpenSans']['h2']),

            Paragraph(_("Project") + ": {}".format(project.name), stylesheets['OpenSans']['Normal']),
            Paragraph(_("Date") + ": {}".format(today), stylesheets['OpenSans']['Normal']),
