...
def collate_itemized_listing(invoice, font, available_width):
    ...
    items = TableFormatter(
        [1, 0, 1, 1, 1, 1], available_width, font, debug=DEBUG_DOCUMENT
    )
    items.style.append(("LEFTPADDING", (0, 0), (-1, -1), 0))
    items.style.append(("RIGHTPADDING", (-1, 0), (-1, -1), 0))
    items.style.append(("VALIGN", (0, 0), (-1, -1), "TOP"))
    items.style.append(
        ("LINEABOVE", (0, "splitfirst"), (-1, "splitfirst"), 0.25, colors.black)
    )
    ...
    def add_total(group):
        ...
        if not group.get("groups") and group.get("tasks"):
            items.row(
                "",
                b("{} {} - {}".format(_("Total"), group["code"], group["name"]), font),
                "",
                "",
                "",
    ...
    for job in invoice["jobs"]:
        ...
        if not group_subtotals_added:
            ...
            totals.row(
                b("{} {} - {}".format(_("Total"), job["code"], job["name"]), font),
...
def collate_payments(invoice, available_width):
    ...
    for payment in invoice["transactions"]:
        ...
        if payment["type"] == "payment":
            ...
            row[0] = Paragraph(
                _("Your Payment on") + " " + received_on, fonts["OpenSans"]["Normal"]
            )
...
def render(project, letterhead, format):
    ...
    with BytesIO() as buffer:
        ...
        flowables = [
            Paragraph(
                _("Itemized listing {}").format(date.today), fonts["OpenSans"]["h2"]
            ),
            Paragraph(
                _("Project") + ": {}".format(project.name), fonts["OpenSans"]["Normal"]
            ),
        ...
        if format == "print":
            ...
            doc.build(
                flowables,
                NumberedLetterheadCanvasWithoutFirstPage.factory(letterhead),
                letterhead,
            )
