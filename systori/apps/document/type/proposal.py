...
from .style import fonts, chunk_text, force_break, p, b, font_name_bold, font_name_normal
...
def collate_tasks(proposal, font, available_width):
    ...
    t.row_style('FONTNAME', 0, -1, font_name_bold(font))
    ...
    for job in proposal['jobs']:
        ...
        for taskgroup in job['taskgroups']:
            ...
            lines = simpleSplit(taskgroup['description'], font_name_normal(font), t.font_size, description_width)
            for line in lines:
                t.row('', p(line, font))
                t.row_style('SPAN', 1, -1)
            ...
            for task in taskgroup['tasks']:
                ...
                lines = simpleSplit(task['description'], font_name_normal(font), t.font_size, description_width)
                for line in lines:
                    t.row('', p(line, font))
                    t.row_style('SPAN', 1, -1)
                    t.row_style('TOPPADDING', 0, -1, 1)
                ...
                t.row('', '', ubrdecimal(task['qty']), p(task['unit'], font)
...
def collate_tasks_total(proposal, font, available_width):
    ...
    t.style.append(('FONTNAME', (0, 0), (-1, -1), font_name_bold(font)))
...
def render(proposal, letterhead, with_line_items, format):
    ...
    with BytesIO() as buffer:
        ...
        flowables = [
            ...
            get_address_label(proposal, letterhead.font),

            get_address_label_spacer(proposal),

            heading_and_date(proposal.get('title') or _("Proposal"), proposal_date, table_width,
                             letterhead.font, debug=DEBUG_DOCUMENT),

            Spacer(0, 4*mm),

            Paragraph(force_break(proposal['header']), fonts[letterhead.font]['Normal']),

            Spacer(0, 4*mm),

            collate_tasks(proposal, letterhead.font, table_width),

            collate_tasks_total(proposal, letterhead.font, table_width),

            Spacer(0, 10*mm),

            KeepTogether(Paragraph(force_break(proposal['footer']), fonts[letterhead.font]['Normal'])),

            ] + (collate_lineitems(proposal, table_width, letterhead.font) if with_line_items else [])
