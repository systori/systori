...
class Timesheet(Document):
    ...
    worker = models.ForeignKey(
        "company.Worker", related_name="timesheets", on_delete=models.CASCADE
    )
    ...
    def calculate_vacation_balance(self):
        ...
        self.json["vacation_net"] = (
            self.json["vacation_added"] - self.json["vacation_total"]
        )
    ...
    def calculate_overtime_balance(self):
        self.json["overtime_net"] = (
            self.json["overtime_total"] - self.json["paid_leave_total"]
        )
    ...
    def calculate_final_compensation(self):
        self.json["compensation_final"] = (
            self.json["compensation_total"] + self.json["work_correction"]
        )

    def calculate_transferred_amounts(self):
        previous_month = self.document_date.replace(day=1) - timedelta(days=2)
        previous_query = Timesheet.objects.period(
    ...
    def calculate(self):
        ...
        self.json.update(
            pdf_type.timesheet.serialize(
                Timer.objects.month(year, month, worker=self.worker).all(), year, month
            )
        )
...
class Proposal(Document):
    ...
    project = models.ForeignKey(
        "project.Project", related_name="proposals", on_delete=models.CASCADE
    )
    jobs = models.ManyToManyField(
        "task.Job", verbose_name=_("Jobs"), related_name="proposals"
    )
...
class Invoice(Document):
    ...
    project = models.ForeignKey(
        "project.Project", related_name="invoices", on_delete=models.CASCADE
    )
    parent = models.ForeignKey(
        "self", related_name="invoices", null=True, on_delete=models.SET_NULL
    )
    ...
    STATE_CHOICES = ((DRAFT, _("Draft")), (SENT, _("Sent")), (PAID, _("Paid")))

    status = FSMField(default=DRAFT, choices=STATE_CHOICES)

    @transition(
        field=status, source=DRAFT, target=SENT, custom={"label": _("Mark Sent")}
    )
    def send(self):
...
class Adjustment(Document):
    ...
    project = models.ForeignKey(
        "project.Project", related_name="adjustments", on_delete=models.CASCADE
    )
    invoice = models.OneToOneField(
        "Invoice", related_name="adjustment", null=True, on_delete=models.SET_NULL
    )
    transaction = models.OneToOneField(
        "accounting.Transaction",
...
class Payment(Document):
    ...
    project = models.ForeignKey(
        "project.Project", related_name="payments", on_delete=models.CASCADE
    )
    invoice = models.OneToOneField(
        "Invoice", related_name="payment", null=True, on_delete=models.SET_NULL
    )
    transaction = models.OneToOneField(
        "accounting.Transaction",
...
class Refund(Document):
    letterhead = models.ForeignKey(
        "document.Letterhead", related_name="refund_documents", on_delete=models.PROTECT
    )
    project = models.ForeignKey(
        "project.Project", related_name="refunds", on_delete=models.CASCADE
    )
    transaction = models.OneToOneField(
        "accounting.Transaction",
        related_name="refund",
...
class DocumentTemplate(models.Model):
    ...
    PROPOSAL = "proposal"
    INVOICE = "invoice"
    DOCUMENT_TYPE = ((PROPOSAL, _("Proposal")), (INVOICE, _("Invoice")))
    document_type = models.CharField(
        _("Document Type"), max_length=128, choices=DOCUMENT_TYPE, default=PROPOSAL
    )
    ...
    def render(self, project=None):
        ...
        result = {"header": self.header, "footer": self.footer}
        for key in result:
            value = result[key]
            for var in vars:
...
class Letterhead(models.Model):
    ...
    letterhead_pdf = models.FileField(
        _("Letterhead PDF"), upload_to="letterhead", max_length=100
    )

    mm = "mm"
    cm = "cm"
    inch = "inch"
    DOCUMENT_UNIT = ((mm, "mm"), (cm, "cm"), (inch, "inch"))
    document_unit = models.CharField(
        _("Document Unit"), max_length=5, choices=DOCUMENT_UNIT, default=mm
    )

    top_margin = models.DecimalField(
        _("Top Margin"), max_digits=5, decimal_places=2, default=Decimal("25")
    )
    right_margin = models.DecimalField(
        _("Right Margin"), max_digits=5, decimal_places=2, default=Decimal("25")
    )
    bottom_margin = models.DecimalField(
        _("Bottom Margin"), max_digits=5, decimal_places=2, default=Decimal("25")
    )
    left_margin = models.DecimalField(
        _("Left Margin"), max_digits=5, decimal_places=2, default=Decimal("25")
    )
    top_margin_next = models.DecimalField(
        _("Top Margin Next"), max_digits=5, decimal_places=2, default=Decimal("25")
    )
    bottom_margin_next = models.DecimalField(
        _("Bottom Margin Next"), max_digits=5, decimal_places=2, default=Decimal("25")
    )
    ...
    document_format = models.CharField(
        _("Pagesize"), max_length=30, choices=DOCUMENT_FORMAT, default=A4
    )

    PORTRAIT = "portrait"
    LANDSCAPE = "landscape"
    ORIENTATION = ((PORTRAIT, _("Portrait")), (LANDSCAPE, _("Landscape")))
    orientation = models.CharField(
        _("Orientation"), max_length=15, choices=ORIENTATION, default=PORTRAIT
    )
    ...
    font = models.CharField(
        _("Font"), max_length=15, choices=FONTS, default=FONTS[0][0]
    )
...
class DocumentSettings(models.Model):
    ...
    proposal_text = models.ForeignKey(
        DocumentTemplate,
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
    ...
    invoice_text = models.ForeignKey(
        DocumentTemplate,
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
    ...
    proposal_letterhead = models.ForeignKey(
        "Letterhead", null=True, blank=True, on_delete=models.SET_NULL, related_name="+"
    )
    invoice_letterhead = models.ForeignKey(
        "Letterhead", null=True, blank=True, on_delete=models.SET_NULL, related_name="+"
    )
    evidence_letterhead = models.ForeignKey(
        "Letterhead", null=True, blank=True, on_delete=models.SET_NULL, related_name="+"
    )
    itemized_letterhead = models.ForeignKey(
        "Letterhead", null=True, blank=True, on_delete=models.SET_NULL, related_name="+"
    )
    timesheet_letterhead = models.ForeignKey(
        "Letterhead", null=True, blank=True, on_delete=models.SET_NULL, related_name="+"
    )
...
class Attachment(models.Model):
    ...
    project = models.ForeignKey(
        "project.Project", null=True, related_name="+", on_delete=models.CASCADE
    )
...
class FileAttachment(models.Model):
    ...
    worker = models.ForeignKey(
        "company.Worker", related_name="+", on_delete=models.PROTECT
    )
