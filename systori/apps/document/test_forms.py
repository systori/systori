...
class TestForm(BaseTestCase):
    ...
    def make_form(self, data=None, initial={}, json=None, txn=None):
        ...
        if txn:
            instance = self.model(
                project=self.project, json=json or {"jobs": []}, transaction=txn
            )
        else:
    ...
    def assert_all_forms_valid(self, form):
        self.assertIsNotNone(
            form.data
        )  # need data in order for is_valid() to even work
        self.assertTrue(
            form.formset.management_form.is_valid(), form.formset.management_form.errors
        )
...
class InvoiceFormTests(TestForm):
    ...
    def test_header_footer_only_set_initially(self):
        invoice_text = DocumentTemplate.objects.create(
            name="default", header="doc settings header", footer="doc settings footer"
        )
...
class ProposalFormTests(TestForm):
    ...
    def test_no_jobs_selected(self):
        ...
        self.assertFalse(form.formset.is_valid())
        self.assertIn(
            "At least one job must be selected.", form.formset.non_form_errors()
        )
