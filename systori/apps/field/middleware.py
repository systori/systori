...
class FieldMiddleware(MiddlewareMixin):
    def process_view(self, request, view, args, kwargs):
        ...
        if "jobsite_pk" in kwargs:
            request.jobsite = JobSite.objects.select_related("project").get(
                pk=kwargs["jobsite_pk"]
            )
            if "dailyplan_url_id" in kwargs:
                ...
                if plan:
                    ...
                    request.dailyplan = DailyPlan(
                        day=date(year, month, day), jobsite=request.jobsite
                    )
    ...
    def process_template_response(self, request, response):
        if request.company and hasattr(response, "context_data"):
            ...
            if (
                hasattr(request, "dailyplan")
                and "dailyplan" not in response.context_data
            ):
