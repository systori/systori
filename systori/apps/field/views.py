...
def _origin_success_url(request, alternate):
    return request.GET.get("origin") or request.POST.get("origin") or alternate


def project_success_url(request):
    return _origin_success_url(
        request, reverse("field.project", args=[request.jobsite.project.id])
    )
...
def dashboard_success_url(request):
    return _origin_success_url(request, reverse("field.dashboard"))


def delete_when_empty(dailyplan):
    if (
        dailyplan.tasks.count() == 0
        and dailyplan.workers.count() == 0
        and dailyplan.equipment.count() == 0
    ):
        dailyplan.delete()
        return True
    return False
...
def daily_plan_objects():
    return (
        ...
        .prefetch_related(
            Prefetch(
                "assigned_equipment",
                queryset=EquipmentAssignment.objects.select_related("equipment"),
            )
        )
...
class FieldDashboard(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if worker.is_fake:
            ...
            previous_plans = (
                worker.dailyplans.filter(day__gt=days_ago(5)).exclude(day=today).all()
            )
        ...
        context.update(
            {
                ...
                "available_holidays": Timer.objects.get_available_vacation(
                    worker=worker
                ),
            }
        )
...
class FieldPlanning(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["previous_day_url"] = reverse(
            "field.planning", args=[context["previous_day"].isoformat()]
        )
        context["today_url"] = reverse(
            "field.planning", args=[date.today().isoformat()]
        )
        context["next_day_url"] = reverse(
            "field.planning", args=[context["next_day"].isoformat()]
        )
        ...
        if context["latest_daily_plan"] is not None:
            context["latest_daily_plan_url"] = reverse(
                "field.planning", args=[context["latest_daily_plan"].day.isoformat()]
            )
        ...
        if selected_day >= date.today():
            context["latest_days_with_plans"] = DailyPlan.objects.values(
                "day"
            ).distinct()[:5]
...
class FieldProjectView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["notes"] = Note.objects.filter(project=project).order_by("-created")[
            :5
        ][::-1]
        context["daily_plans"] = grouped_by_days
        context["latest_daily_plan"] = (
            daily_plan_objects().filter(jobsite__in=project.jobsites.all()).first()
        )
        context["today"] = date.today()
        context["latest_daily_plan_global"] = DailyPlan.objects.order_by("-id").first()
        if context["latest_daily_plan_global"] is not None:
            context["latest_daily_plan_global_url"] = reverse(
                "field.planning",
...
class FieldNoteList(ListView):
    ...
    def post(self, request, *args, **kwargs):
        if self.kwargs.get("project_pk", None):
            project = Project.objects.get(id=self.kwargs["project_pk"])
            note = Note(project=project, content_object=project, worker=request.worker)
            form = NoteForm(request.POST, instance=note)
            if form.is_valid():
                form.save()
                return HttpResponseRedirect(
                    reverse("field.project", kwargs={"project_pk": project.pk})
                )
...
class FieldHTMLCalendar(LocaleHTMLCalendar):
    ...
    def formatday(self, day, weekday):
        if day == 0:
            ...
            return (
                '<td class="%s"><a href="%s">%d <span class="dailyplan-count">%s</span></a></td>'
                % (
...
class FieldProjectCalendar(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["calendar"] = FieldHTMLCalendar(
            ...
            previous,
            context["next_month"],
            lambda day_date: reverse(
                "field.project", args=[project.id, day_date.isoformat()]
            )
            + "?copy_source_date="
            + self.request.GET.get("copy_source_date", ""),
        ).render()
...
class FieldCopyPasteDailyPlans(View):
    def get(self, request, *args, **kwargs):
        ...
        for oldplan in DailyPlan.objects.filter(
            jobsite__project=project, day=other_day
        ):

            newplan = DailyPlan.objects.create(
                jobsite=oldplan.jobsite, day=selected_day, notes=oldplan.notes
            )
        ...
        return HttpResponseRedirect(
            reverse("field.project", args=[project.id, selected_day.isoformat()])
        )
        ...
        for oldplan in DailyPlan.objects.filter(day=source_day):
            ...
            for oldequipment in oldplan.assigned_equipment.all():
                EquipmentAssignment.objects.create(
                    dailyplan=newplan, equipment_id=oldequipment.equipment_id
                )

        return HttpResponseRedirect(
            reverse("field.planning", args=[selected_day.isoformat()])
        )
...
class FieldGroupView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["is_parent_job"] = (
            isinstance(self.object, Group) and self.object.is_root
        )
...
class FieldTaskView(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["in_current_dailyplan"] = (
            dailyplan.id and dailyplan in task.dailyplans.all()
        )
        return context
    ...
    def assign_task_to_user_dailyplan(self, task):
        ...
        worker = self.request.worker
        dailyplan_assigned = (
            dailyplan.id and dailyplan.members.filter(worker=worker).exists()
        )
...
class FieldAddSelfToDailyPlan(View):
    def get(self, request, *args, **kwargs):
        ...
        already_assigned = TeamMember.objects.filter(
            dailyplan=dailyplan, worker=self.request.worker
        ).exists()
...
class FieldRemoveSelfFromDailyPlan(View):
    def get(self, request, *args, **kwargs):
        TeamMember.objects.filter(
            dailyplan=request.dailyplan, worker=request.worker
        ).delete()
...
class FieldAssignLabor(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        workers = (
            Worker.objects.prefetch_related("user")
    ...
    def post(self, request, *args, **kwargs):
        ...
        for worker in new_assignments:
            if worker not in previous_assignments:
                TeamMember.objects.create(dailyplan=dailyplan, worker_id=worker)

        # Remove unchecked assignments
        for worker in previous_assignments:
            if worker not in new_assignments:
                TeamMember.objects.filter(
                    dailyplan=dailyplan, worker_id=worker
                ).delete()
...
class FieldPlanningToggle(View):
    def get(self, request, *args, **kwargs):
        ...
        return HttpResponseRedirect(
            reverse("field.planning", args=[self.request.selected_day.isoformat()])
        )
...
class FieldAssignEquipment(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        dailyplan = self.request.dailyplan
        if dailyplan.id:
            context["assigned"] = dailyplan.equipment.all()
        return context
    ...
    def post(self, request, *args, **kwargs):
        ...
        for equipment in new_assignments:
            if equipment not in previous_assignments:
                EquipmentAssignment.objects.create(
                    dailyplan=dailyplan, equipment_id=equipment
                )
