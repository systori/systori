...
class NoteTest(ClientTestCase):
    ...
    def test_update_note_with_success_url(self):
        ...
        response = self.client.post(
            reverse("note", args=[note.pk, "/project-{}".format(self.project.id)]),
    ...
    def test_field_note(self):
        ...
        note2 = NoteFactory(
            project=self.project2, content_object=self.project2, worker=self.worker
        )
        ...
        response = self.client.get(
            reverse("field.notes", kwargs={"project_pk": self.project.pk})
        )
        self.assertIn(note1, response.context["object_list"])
