...
class EquipmentForm(forms.ModelForm):
    last_refueling_stop = forms.DateField(
        label=_("last_refueling_stop"), disabled=True, required=False
    )
...
class RefuelingStopForm(forms.ModelForm):
    ...
    average_consumption = forms.DecimalField(
        label=_("average_consumption"), disabled=True, required=False
    )
    ...
    def clean_mileage(self):
        ...
        older_refueling_stop = (
            RefuelingStop.objects.filter(
                equipment=self.cleaned_data.get("equipment").id, mileage__gt=mileage
            )
            .order_by("-mileage")
            .first()
        )
        if self.instance.pk:
            if (
                self.instance.older_refueling_stop
                and mileage <= self.instance.older_refueling_stop.mileage
            ):
                raise ValidationError(
                    _("you must enter a higher mileage than the older refueling stop.")
                )
            elif (
                self.instance.younger_refueling_stop
                and mileage >= self.instance.younger_refueling_stop.mileage
            ):
                raise ValidationError(
                    _(
