...
{{instance.code}}
    & \multicolumn{4}{l}{\parbox[t]{9cm}{ {{instance.full_name|tex_escape}} }} \\*
    & \multicolumn{4}{l}{\parbox[t]{9cm}{ {{instance.full_description|tex_escape}} }} \\*
    & & {{task.qty|floatformat:2}} & {{task.unit|tex_escape}} & {{instance.unit_price|floatformat:2}} €  & {% if task.is_optional %}{% trans 'Optional' %}{% elif not instance.selected %}{% trans 'alternate'%} {% else %}{{instance.fixed_price_estimate|floatformat:2}} €{% endif %} \\
{% endfor %}
{% endfor %}

\tabucline{6}
& \multicolumn{4}{r} {\textbf{ {% trans 'Total' %} {{job.code}}.{{forloop.counter}} - {{taskgroup.name|tex_escape}} }} & \textbf{ {{taskgroup.estimate_total|floatformat:2}} € }\\
\\
{% endfor %}
