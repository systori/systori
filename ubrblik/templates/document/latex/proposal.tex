...
\begin{flushright}
  {% trans 'Date' %}: {% now "l" %}, {% now "d" %}. {% now "F Y" %}
\end{flushright}
{{ doc.header }}
...
{{instance.code}}
    & \multicolumn{4}{l}{\parbox[t]{9cm}{ {{instance.full_name}} }} \\*
    & \multicolumn{4}{l}{\parbox[t]{9cm}{ {{instance.full_description}} }} \\*
    & & {{task.qty|floatformat:2}} & {{task.unit}} & {{instance.unit_price|floatformat:2}} €  & {% if task.is_optional %}{% trans 'Optional' %}{% elif not instance.selected %}{% trans 'alternate'%} {% else %}{{instance.fixed_price_estimate|floatformat:2}} €{% endif %} \\
{% endfor %}
{% endfor %}

\tabucline{6}
& \multicolumn{4}{r} {\textbf{ {% trans 'Total' %} {{job.code}}.{{forloop.counter}} - {{taskgroup.name}} }} & \textbf{ {{taskgroup.estimate_total|floatformat:2}} € }\\
\\
{% endfor %}
