...
def migrate_accounts(company):
    ...
    for project in Project.objects.without_template().order_by("id"):
        ...
        for i_invoice, invoice in enumerate(project.invoices.all()):
            ...
            print(
                "\n Invoice #{} - {} - {}".format(
                    invoice.id, invoice.invoice_no, invoice.document_date.isoformat()
                )
            )
            ...
            for criteria in match_criteria:
                ...
                matches = (
                    Transaction.objects.filter(entries__account=project.account)
                    .filter(**criteria)
                    .distinct()
                )
                if matches.count() == 1:
            ...
            for json_job in invoice.json.pop("jobs"):
                ...
                job = project.jobs.get(
                    name=json_job["name"], job_code=int(json_job["code"])
                )
            ...
            print(
                "  {:<50} {:>15} {:>15}".format(
                    "", money(invoice_amount), money(pre_tax_invoice)
                )
            )
            if round(invoice.amount, 2) != round(invoice_amount, 2):
                ...
                if (
                    invoice.id
                    in [31, 37, 38, 46, 47, 51, 54, 55, 56, 60, 62, 63, 67, 69, 70, 71]
        ...
        for payment in project.account.payments().all():
            ...
            print(
                "\n Converting Payment Transaction #{} (entries: {}) - {}".format(
                    transaction.id, len(entries), money(abs(payment.amount))
                )
            )
            ...
            invoice = (
                Invoice.objects.filter(
                    project=project, document_date__lt=transaction.transacted_on
                )
            ...
            if invoice and invoice.amount >= (
                ...
                print(
                    "\n   Applying payment to previous invoice #{} - {} - {} - {}...".format(
                        invoice.id,
                        invoice.invoice_no,
                ...
                for debit_idx, debit in enumerate(non_zero_debits):
                    ...
                    assert (
                        job_credit + job_discount - Decimal("0.01")
                    ) <= job.account.balance
                    ...
                    transaction.credit(
                        job.account, job_credit, entry_type=Entry.PAYMENT, job=job
                    )
                    transaction.debit(
                        Account.objects.get(code="1710"), job_credit, job=job
                    )
                    transaction.credit(
                        Account.objects.get(code="1718"), job_income, job=job
                    )
                    transaction.credit(
                    ...
                    if discount_entry:
                        ...
                        transaction.debit(
                            Account.objects.get(code="1710"), job_discount, job=job
                        )
                ...
                for idx, (job_percent, job_balance, job) in enumerate(sorted_jobs):
                    ...
                    if idx == last_job_idx:  # use whatever is left on last job
                        ...
                        assert (
                            round(1 - job_credit / (job_credit + job_discount), 3)
                            == discount_percent
                        )
