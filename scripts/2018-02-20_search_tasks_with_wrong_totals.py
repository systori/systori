...
for task in Task.objects.all():
    ...
    for li in task.lineitems.all():
        new_total += li.total
        if (li.total_equation and is_formula(li.total_equation))\
    ...
    if new_total != task.price:
        ...
        if new_total - task.price > Decimal("0.01"):
            ...
            logging['task_real_mismatch'].append((task.code, task.id, task.job.project.id, task.price, new_total))
...
""".format(counter_task_uses_equation, counter_lineitem_uses_equation, counter_including_rounding_mismatch, counter_real_mismatch)
