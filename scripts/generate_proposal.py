...
LineItemFactory(name="Remove brush", unit="hr", unit_qty=8, price=12, task=level_task)
LineItemFactory(name="Level", unit="hr", unit_qty=4, price=26, task=level_task)
LineItemFactory(name="Compact", unit="hr", unit_qty=4, price=20, task=level_task)
concrete_task = TaskFactory(name="Pour Concrete Pad", group=booth_group, qty=1, description=desc)
LineItemFactory(name="Forms", unit="ft", unit_qty=500, price=3, task=concrete_task)
LineItemFactory(name="Setup Forms", unit="hr", unit_qty=8, price=20, task=concrete_task)
LineItemFactory(name="Concrete", unit="cu ft", unit_qty=200, price=5, task=concrete_task)
LineItemFactory(name="risk", unit="%!:", unit_qty=10, price=2660, task=concrete_task)
LineItemFactory(name="Screed", unit="hr", unit_qty=2, price=20, task=concrete_task)
LineItemFactory(name="Remove Forms", unit="hr", unit_qty=4, price=20, task=concrete_task)
LineItemFactory(name="risk", unit="%!]&", unit_qty=10, price=120, task=concrete_task)
LineItemFactory(name="discount", unit="%!:", unit_qty=-10, price=3058, task=concrete_task)
booth_task = TaskFactory(name="Install Booth", group=booth_group, qty=1, description=desc)
LineItemFactory(name="Manufacturer suggested installation cost", unit="", unit_qty=1, price=5000, task=booth_task, is_hidden=True)
LineItemFactory(name="Place ontop concrete pad", unit="hr", unit_qty=2, price=50, task=booth_task)
...
LineItemFactory(name="", unit="hr", unit_qty=24, price=40, task=excavate_task)
foundation_task = TaskFactory(name="Walls", group=foundation_group, qty=1)
LineItemFactory(name="Forms", unit="ft", unit_qty=500, price=3, task=foundation_task)
LineItemFactory(name="Setup Forms", unit="hr", unit_qty=8, price=20, task=foundation_task)
LineItemFactory(name="Concrete", unit="cu ft", unit_qty=200, price=5, task=foundation_task)
LineItemFactory(name="Remove Forms", unit="hr", unit_qty=4, price=20, task=foundation_task)
