...
security_group = GroupFactory(
    name="Security Checkpoint", parent=entrance_group, description=desc
)
GroupFactory(
    name="Boom Gate",
    parent=GroupFactory(name="", parent=security_group),
    description=desc,
)
booth_group = GroupFactory(
    name="Booth", parent=GroupFactory(name="", parent=security_group), description=desc
)
level_task = TaskFactory(
    name="Level Area", group=booth_group, qty=1, total=1, description=desc
)
...
concrete_task = TaskFactory(
    name="Pour Concrete Pad", group=booth_group, qty=1, total=1, description=desc
)
LineItemFactory(name="Forms", unit="ft", total=500, price=3, task=concrete_task)
LineItemFactory(name="Setup Forms", unit="hr", price=8, total=20, task=concrete_task)
LineItemFactory(name="Concrete", unit="cu ft", qty=200, price=5, task=concrete_task)
LineItemFactory(
    name="risk", unit="%", qty=10, price_equation="C!:", price=2660, task=concrete_task
)
...
LineItemFactory(
    name="risk", qty=10, unit="%", price_equation="C!]&", price=120, task=concrete_task
)
LineItemFactory(
    name="discount",
    qty=-10,
    unit="%",
    price_equation="1000.001*C!:",
    price=3058,
    task=concrete_task,
)
booth_task = TaskFactory(
    name="Install Booth", group=booth_group, qty=1, total=1, description=desc
)
LineItemFactory(
    name="Manufacturer suggested installation cost",
    unit="",
    qty=1,
    price=5000,
    task=booth_task,
...
LineItemFactory(
    name="Place ontop concrete pad", unit="hr", qty=2, price=50, task=booth_task
)
...
excavate_task = TaskFactory(name="Excavate", group=foundation_group, total=1, qty=1)
LineItemFactory(name="", unit="hr", qty=24, price=40, task=excavate_task)
foundation_task = TaskFactory(name="Walls", group=foundation_group, total=1, qty=1)
