...
def parse_p83_file(filename):
    ...
    for _ in LVBereich.finditer(content):
        ...
        for job in LVBereich.finditer(_.group("content")):
            ...
            job_dict["attrs"]["Langtext"] = re.sub(
                r" {2,}", " ", rtf_to_text(job_dict["attrs"]["Langtext"])
            )
            ...
            for taskgroup in LVBereich.finditer(job_content):
                ...
                taskgroup_dict["attrs"]["Langtext"] = re.sub(
                    r" {2,}", " ", rtf_to_text(taskgroup_dict["attrs"]["Langtext"])
                )
                ...
                for task in Position.finditer(taskgroup_content):
                    ...
                    task_dict["attrs"]["Langtext"] = re.sub(
                        r" {2,}", " ", rtf_to_text(task_dict["attrs"]["Langtext"])
                    )
        ...
        for taskgroup in job["taskgroups"]:
            ...
            for task in taskgroup["tasks"]:
                ...
                _task = Task.objects.create(
                    ...
                    qty=decimal.Decimal(
                        task["attrs"]["Menge"].replace(".", "").replace(",", ".")
                    ),
                    unit=task["attrs"]["ME"],
                )
