    ...
    for company in Company.objects.all():
        ...
        for invoice in Invoice.objects.all():
            ...
            jobs = Job.objects.filter(
                id__in=[d["job.id"] for d in invoice.json["debits"]]
            )
            ...
            if (
                invoice.id not in [44, 46, 63, 69, 74, 75, 76, 77, 78, 81]
                and company.name != "Demo"
            ):
            ...
            if company.name != "Demo" and (
                abs(D(report["debited_net"]) - round(D(invoice.json["debited_net"]), 2))
                > 0
                or abs(
                    D(report["debited_gross"])
                    - round(D(invoice.json["debited_gross"]), 2)
                )
                > 0
            ):
                invoice_idx += 1
                for field in ["debited_gross", "debited_net", "debited_tax"]:
                    ...
                    if diff != 0:
                        print(
                            "{} | [{p}](https://systori.com/project-{pid}) | [{i}](https://systori.com/project-{pid}/invoice-print-{iid}.pdf) | {} | {} | {} | {}".format(
                                invoice_idx,
                                field,
                                old,
                                new,
                                ...
                                i=invoice.json["title"],
                                iid=invoice.id,
                            )
                        )

            if company.name != "Demo":
                assert D(report["transactions"][-1]["gross"]) == round(
                    D(invoice.json["debit_gross"]), 2
                )
            ...
            for job in invoice.json["debits"]:
                ...
                job["debited_net"], job["debited_tax"] = extract_net_tax(
                    D(job["debited"]), D("0.19")
                )
                job["balance_gross"] = job["balance"]
                job["balance_net"], job["balance_tax"] = extract_net_tax(
                    D(job["balance"]), D("0.19")
                )
        ...
        for proposal in Proposal.objects.all():
            ...
            if "total_gross" not in proposal.json or (
                ...
                proposal.json["total_net"], job["total_tax"] = extract_net_tax(
                    D(proposal.amount), D("0.19")
                )
                proposal.save()
